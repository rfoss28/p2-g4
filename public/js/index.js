...
$(function () {
  ...
  $("#medicInput").on("click", function (event) {
    ...
    for (i = 0; i < rxList.length; i++) {
      var rxQuery = "https://rxnav.nlm.nih.gov/REST/rxcui.json?name=" + rxList[i];
    ...
    Promise.all(promises)
      ...
      .then(function (data) {
        ...
        data.forEach((elem, index) => console.log(`this is the ${index} elem  ${elem.idGroup.rxnormId[0]}`));

        //load codes into code array
        data.forEach((elem, index) => codeArray.push(`${elem.idGroup.rxnormId[0]}`));


        //everything successful, handle data here
        // data is array of results IN ORDER they were passed

        var queryURL = "https://rxnav.nlm.nih.gov/REST/interaction/list.json?rxcuis=";
        ...
        $.ajax({
          ...
          response.fullInteractionTypeGroup.forEach(function (j, index) {
            ...
            j.fullInteractionType.forEach(function (i) {
              ...
              var nameArray = i.interactionPair[0].interactionConcept.map(function (rxNames) {
                // i.interactionPair[0].sourceConceptItem.forEach(function(rxNames){
                medNames += rxNames.sourceConceptItem.name + " ";
                // console.log(nameArray);
              });
              
              interactions += medNames + "Interaction Description: " + i.interactionPair[0].description + " Severity: " + severity.toUpperCase() + "\r\n";
